...
int handle_fuzz(int argc, char *argv[])
{
    ...
    snprintf(stderr_path, sizeof(stderr_path), "%s/target_stderr.log", normal_out);
    snprintf(stdout_path, sizeof(stdout_path), "%s/target_stdout.log", normal_out);

    int fd = open(stderr_path, O_CREAT | O_WRONLY, 0666);
    if (fd >= 0)
    {
    ...
    int fd_out = open(stdout_path, O_CREAT | O_WRONLY, 0666);
    if (fd_out >= 0)
    {
